...
「저장하지 말아야 할 것을 담는 전용 컬럼이 없는가」 — 자연어 원문·전체 프롬프트용 컬럼 이름이
어느 테이블에도 없어야 한다(ADR-002). 이 검사가 보장하는 범위는 컬럼 목록이다. TEXT·JSONB
컬럼에 원문이 들어가지 않는 것은 쓰기 경로가 원문을 넘기지 않는 것으로 지키며, 여기서는 검사하지 않는다.
"""
import uuid
