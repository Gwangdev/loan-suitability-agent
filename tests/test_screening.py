...
from loan_agent import demo as demo_cases, parser, screening

ALL_CASES = demo_cases.TEST_CASES + demo_cases.EDGE_CASES


@pytest.mark.parametrize("tc", ALL_CASES, ids=[c["name"] for c in ALL_CASES])
def test_verdict_matches_expected(tc):
    parsed = parser.rule_based_parse(tc["input"])
    result = screening.screen_loan(parsed)
    assert result["판정"] == tc["expected"]


def test_all_three_verdicts_represented():
    """데모가 승인가능/상담필요/어려움 세 판정을 모두 보여주는지."""
    verdicts = {tc["expected"] for tc in demo_cases.TEST_CASES}
...
def test_limit_boundary():
    ...
    at_limit = screening.screen_loan({**base, "희망금액": 7000000})
    over_limit = screening.screen_loan({**base, "희망금액": 7000001})
...
def test_zero_income_is_insufficient():
    r = screening.screen_loan({"월소득": 0, "부채": 0, "신용등급": 1,
...
def test_negative_income_clamped_to_insufficient():
    r = screening.screen_loan({"월소득": -500, "부채": 0, "신용등급": 1,
                          "희망금액": 1000000, "직장유형": "정규직"})
    assert r["상환능력"] == "부족"
...
def test_dsr_increases_with_loan_amount():
    ...
    small = screening.screen_loan({**base, "희망금액": 10000000})
    large = screening.screen_loan({**base, "희망금액": 100000000})
...
def test_monthly_payment_zero_principal():
    assert screening.monthly_payment(0) == 0.0


def test_monthly_payment_zero_rate_is_simple_division():
    assert screening.monthly_payment(1200000, annual_rate=0.0, months=12) == 100000.0


def test_monthly_payment_positive():
    assert screening.monthly_payment(10000000) > 0


# ── '어려움'이면 추천/후보 없음 ─────────────────────────────
def test_hard_verdict_has_no_recommendation():
    hard = screening.screen_loan({"월소득": 1800000, "부채": 30000000, "신용등급": 6,
