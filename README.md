...
라이브 안내문 결과 화면은 `usage_metrics`의 입력·출력 토큰과 추정 API 비용을 함께 표시한다.
비용은 캐시 입력 할인 없이 일반 입력 단가를 적용한다.
...
| 단가 기준일 | 2026-08-28 |
| 입력 토큰 상수 | `TOKEN_INPUT_USD_PER_MILLION` |
| 출력 토큰 상수 | `TOKEN_OUTPUT_USD_PER_MILLION` |
| 출처 | [OpenAI 공식 모델 문서](https://developers.openai.com/api/docs/models/gpt-4o-mini) |

측정 당시 로컬 PostgreSQL 테스트 DB에는 `explanation_run` 97,661건이 있었지만 토큰이 완전히 기록된
실행은 0건이었다. 따라서 아래 수치는 현재 운영 평균이 아니라 2026-07-27에 생성된 사전 녹화
5개 파이프라인의 누적 `usage_metrics` 스냅샷을 회차별 차분한 관측치다.

| 산출 | 관측값 |
|---|---:|
| 요청당 평균 입력 토큰 | 27,653.4 |
| 요청당 평균 출력 토큰 | 4,762.2 |
| 요청당 추정 비용 | $0.00700533 |
| 월 10,000건 추정 비용 | $70.0533 |

계산식은 `(입력 토큰 × 0.15 + 출력 토큰 × 0.60) / 1,000,000`이며 월 비용은 요청당 비용에
월 요청 수를 곱한다. 녹화 데이터의 `successful_requests` 차분은 각 파이프라인 실행마다 15건으로,
3개의 단계 라벨이 곧 3회의 제공자 호출이라는 뜻은 아니다. ReAct 도구 재조회와 프레임워크 실행이
추가 호출로 포함될 수 있으므로, 실제 비용은 `explanation_run`에 기록된 토큰을 우선 사용해야 한다.

## 한계와 미검증 범위

이 저장소는 교육용 의사결정 지원 데모이며, 실제 대출 승인·거절이나 상품 중개를 수행하지 않는다.
입력과 상품 데이터는 합성이고, DSR은 대표 금리·기간과 제한된 부채 범위를 가정한 단순화 모델이다.
Eval의 100% 통과는 녹화된 케이스와 여섯 규칙 지표 안에서만 성립하며, 자연스러운 표현 품질이나
실시간 LLM 제공자 장애를 보장하지 않는다. 법적·보안 통제의 범위와 실서비스 전환 시 남는 과제는
[리스크 통제 대장](docs/리스크_통제_대장.md)을 기준으로 한다.
...
- [x] **하이브리드 입력 UX(C-3)** — 자유서술 → 파싱 → 필드별 폼 자동채움 → 부족분 강조 → 폼으로 제출.
  판정을 구조화 값으로 계산해 화면/안내문 불일치(A1)를 해소하고, "결정적 심사" 버튼으로 **키 없이도** 판정 확인.
- [ ] **성능 최적화 캐싱(C-1)** — v2 후속(결과 캐시 + 프롬프트 캐싱).
