...
def _render_data_notice():
    """입력 전에 데이터 취급 방식을 알린다.

    이 서비스는 합성 상품표로 도는 교육용 데모이고, 실제 개인신용정보를 받는 순간
    개인정보보호법·신용정보법의 적용 조건 안으로 들어간다. 조건 밖에 있도록 설계한
    것이 산출물이므로, 그 전제를 사용자에게 알리지 않으면 전제가 성립하지 않는다.

    네 가지를 적는다 — 넣지 말아야 할 것, 상품 데이터의 출처, 저장되는 것,
...
def main():
    ...
    # 입력을 받기 전에 무엇을 넣으면 안 되고 무엇이 저장되는지 알린다. 리스크 통제 대장이
    # 이 고지를 통제 항목으로 적어 두었는데 화면에는 렌더되지 않아, 문서가 약속한 것과
    # 실제 실행 경로가 갈라져 있었다. 제출 버튼 뒤가 아니라 앞에 두어야 고지 구실을 한다.
    _render_data_notice()
