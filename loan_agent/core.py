...
def load_products(csv_path: Path = CSV_PATH) -> list:
    ...
    for line in lines[1:]:
        ...
        products.append({
            ...
            # [디벨롭: 다기준 랭킹] 금융감독원 「금융상품 한눈에」 공시가 실제로 노출하는 필드를
            #   합성값으로 추가(상환방식·금리방식·중도상환수수료). 최저금리 단일 기준을 넘어선
            #   상품 랭킹(§docs 기술부채_보안_개선계획 C-2)을 시연하기 위한 데이터.
...
def screen_loan(customer: dict, products: list = None) -> dict:
    ...
    # [디벨롭: 다기준 랭킹] 예전엔 적격 상품을 '최저금리' 단일 기준으로만 골랐다.
    ...
    return {
        ...
        "추천상품": 추천,
        # [디벨롭: 다기준 랭킹] 상위 3개 랭킹(비교·시연용). 안내문/UI에서 대안 상품 비교에 쓸 수 있다.
        "추천후보": 추천후보,
