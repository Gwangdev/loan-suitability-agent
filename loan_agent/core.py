...
def screen_loan(customer: dict, products: list = None) -> dict:
    ...
    참고(DTI 지표의 한계): 여기서 DTI는 "기존 부채 잔액 ÷ 연소득"의 간이 프록시다.
    금융위 기준 실제 DTI/DSR(연간 원리금상환액 ÷ 연소득)과는 정의가 다르고, 이번에 신청하는
    희망금액 자체의 상환 부담은 반영하지 않는다(그래서 기존 부채가 0이면 희망금액과 무관하게
    항상 "여유"로 나온다). 교육용 데모의 단순화이며, 실제 서비스라면 희망금액에 가정 금리·기간을
    곱한 월상환액을 반영하도록 보강해야 한다."""
    if products is None:
        products = PRODUCTS
    ...
    # (2) 상환능력(DTI)
    연소득 = 월소득 * 12
    dti = (부채 / 연소득) if 연소득 > 0 else float("inf")
    if dti <= 0.30:
        상환 = "여유"
    elif dti <= 0.60:
        상환 = "보통"
...
TEST_CASES = [
    {"name": "승인 케이스",
     ...
     "expected": "승인가능"},
    {"name": "상담필요 케이스",
     "input": "월소득 280만원이고 부채가 1500만원 있어요. 신용등급 4등급, 2000만원 대출 희망합니다.",
     "expected": "상담필요"},
    {"name": "어려움 케이스",
...
def run_logic_selftest(cases: list = None) -> bool:
    ...
    for tc in cases:
        ...
        print(f"\n[{tc['name']}] {'✅' if ok else '❌'}")
        print(f"  파싱 : {parsed}")
        print(f"  기대 : {tc['expected']} / 판정: {result['판정']} (상환 {result['상환능력']}, DTI {result['DTI']})")
...
def build_reviewer_agent(llm):
    ...
    return Agent(
        ...
        backstory=(
            ...
            "그 위에서 CoT로 (1)부채비율 (2)신용등급 조건 (3)희망금액 대비 한도 (4)적합상품 선별을 단계적으로 설명하고, "
...
def build_tasks(parser, reviewer, advisor, on_stage=None):
    ...
    review_task = Task(
        description=(
            ...
            "그런 다음 CoT 4단계((1)부채비율 (2)신용등급 조건 (3)희망금액 대비 한도 (4)적합상품 선별)로 근거를 설명하고, "
        ...
        expected_output="최종 판정(승인가능/상담필요/어려움), CoT 4단계 근거, SC 교차검증 요약, 도구의 '추천상품' 필드(상품코드·은행명 포함, 어려움이면 None) 그대로, 적격/부적격 상품과 사유, DTI 등 핵심 계산값.",
