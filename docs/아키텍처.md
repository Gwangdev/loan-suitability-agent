...
flowchart LR
    ...
    Worker -. 서버 키를 이용한 호출 의도 .-> Provider
```
...
**로컬 구성의 제약:** 공통 Compose에서 worker는 `internal: true`인 backend에만 연결된다. 그림의 점선은 코드가 의도하는 외부 호출이며, 이 설정만으로 외부 OpenAI 연결이 가능하다고 보장할 수 없다. 컨테이너 워커의 라이브 실행에는 외부 통신 경로 확인이 필요하다. 또한 서버 키가 없는데 워커를 켜면 작업을 확보한 후 `PROVIDER_ERROR`로 실패할 수 있다. 이 문서 작업에서는 네트워크·배포 설정을 변경하지 않았다.
