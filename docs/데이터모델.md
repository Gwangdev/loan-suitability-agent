...
**함께 고친 것(대장 `#29`).** 모델의 타입 힌트가 `Mapped[float]`였는데 SQLAlchemy `Numeric`은 `asdecimal=True`가 기본이라 **읽을 때 `Decimal`을 돌려준다.** 쓸 때는 `float`가 들어가므로 힌트가 왕복의 절반만 참이었다. `Mapped[Decimal]`로 고치고, **DB 왕복 타입을 단언하는 통합 테스트**를 함께 넣었다 — 타입 힌트만 고치면 같은 오류가 다시 들어온다.
